...
function StackedBar({ reason, data, metric, segs, pct }) {
  ...
  useEffect(() => {
    if (!ref.current||!window.Chart||!window.ChartDataLabels) return
...
export default function App() {
  ...
  const [mainTab, setMainTab]       = useState("dashboard")  // dashboard | editor
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        const { rows, fileName: fn } = JSON.parse(saved)
        if (rows?.length) applyData(rows, fn)
      }
    } catch(e) {}
  }, [])

  // Save to localStorage whenever raw changes
  useEffect(() => {
    if (raw.length) {
      try { localStorage.setItem("dmp_data", JSON.stringify({ rows: raw, fileName })) } catch(e) {}
    }
  }, [raw, fileName])
  ...
  function CmpTable({ title, list }) {
    const cb={ padding:"7px 10px", textAlign:"right", whiteSpace:"nowrap", fontVariantNumeric:"tabular-nums", fontSize:12, borderBottom:"0.5px solid var(--color-border-tertiary)", color:"var(--color-text-primary)" }
    const lb={ padding:"7px 10px", textAlign:"left", whiteSpace:"nowrap", fontSize:12, borderBottom:"0.5px solid var(--color-border-tertiary)", color:"var(--color-text-primary)" }
    return (
          ...
          <table style={{width:"100%",fontSize:12,borderCollapse:"collapse"}}>
            <thead>
              <tr style={{background:"var(--color-background-secondary)"}}>
                <th style={{...lb,fontSize:11,color:"var(--color-text-secondary)",textTransform:"uppercase",letterSpacing:"0.04em"}}></th>
                ...
                {ordSegs.map(s=>(
                  modelWt
                    ...
                    : <th key={s} colSpan={2} style={{...cb,fontSize:11,color:SEG_COLORS[SEGMENT_COLS.indexOf(s)],textTransform:"uppercase",letterSpacing:"0.04em",borderLeft:"1px solid var(--color-border-tertiary)"}}><span style={{display:"inline-flex",alignItems:"center",gap:4,justifyContent:"flex-end"}}><span style={{width:7,height:7,borderRadius:2,background:SEG_COLORS[SEGMENT_COLS.indexOf(s)]}}></span>{dn(s)}</span></th>
                ))}
              </tr>
              {!modelWt && (
                <tr style={{background:"var(--color-background-secondary)"}}>
                  <th style={{...lb,fontSize:10,color:"var(--color-text-tertiary)"}}></th>
                  {ordSegs.map(s=>[
                    <th key={s+"-wt"} style={{...cb,fontSize:10,color:"var(--color-text-tertiary)",borderLeft:"1px solid var(--color-border-tertiary)"}}>Weight</th>,
                    <th key={s+"-sr"} style={{...cb,fontSize:10,color:"var(--color-text-tertiary)"}}>Save rate</th>
                  ])}
                </tr>
              )}
            </thead>
            <tbody>
              {list.map((cr,i)=>(
                <tr key={cr} style={{background:i%2===0?"var(--color-background-primary)":"var(--color-background-secondary)"}}>
                  <td style={lb}>{cr}</td>
                  ...
                  {ordSegs.map(s=>{
                    ...
                    if (modelWt) return <td key={s} style={{...cb,color:bad?"#C0392B":"var(--color-text-primary)",fontWeight:bad?500:400}}>{fmtP(v)}</td>
                    return [
                      <td key={s+"-wt"} style={{...cb,color:"#5D93FF",borderLeft:"1px solid var(--color-border-tertiary)"}}>{fmtP(mWeight(cr,list,s))}</td>,
                      <td key={s+"-sr"} style={{...cb,color:bad?"#C0392B":"var(--color-text-primary)",fontWeight:bad?500:400}}>{fmtP(v)}</td>
                    ]
                  })}
                </tr>
              ))}
              <tr><td colSpan={modelWt?2+ordSegs.length:1+ordSegs.length*2} style={{padding:"2px 0",background:"var(--color-background-tertiary)"}}></td></tr>
              <tr style={{background:"var(--color-background-secondary)"}}>
                ...
                {ordSegs.map(s=>{
                  ...
                  if (modelWt) return <td key={s} style={{...cb,fontWeight:600,color:bad?"#C0392B":"var(--color-text-primary)"}}>{fmtP(v)}</td>
                  return [
                    <td key={s+"-wt"} style={{...cb,fontWeight:600,color:"#5D93FF",borderLeft:"1px solid var(--color-border-tertiary)"}}>—</td>,
                    <td key={s+"-sr"} style={{...cb,fontWeight:600,color:bad?"#C0392B":"var(--color-text-primary)"}}>{fmtP(v)}</td>
                  ]
                })}
              </tr>
            </tbody>
    ...
    <div style={{padding:"1rem",maxWidth:1400,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"1.25rem",flexWrap:"wrap",gap:8}}>
        <div>
          ...
          <div style={{fontSize:12,color:"var(--color-text-tertiary)",marginTop:2}}>
            {fileName || "No data loaded"}{raw.length ? ` · ${rawCats.length} cancellation reasons` : ""}
          </div>
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",overflow:"hidden"}}>
            {[["dashboard","Dashboard"],["editor","Edit Data"]].map(([k,l])=>(
              <button key={k} onClick={()=>setMainTab(k)} style={{padding:"5px 12px",fontSize:12,border:"none",cursor:"pointer",background:mainTab===k?"var(--color-background-secondary)":"var(--color-background-primary)",color:"var(--color-text-primary)",fontWeight:mainTab===k?500:400}}>{l}</button>
            ))}
          </div>
          {mainTab==="dashboard" && (
            <div style={{display:"flex",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",overflow:"hidden"}}>
              {["charts","table"].map(v=>(
                <button key={v} onClick={()=>setView(v)} style={{padding:"5px 12px",fontSize:12,border:"none",cursor:"pointer",background:view===v?"var(--color-background-secondary)":"var(--color-background-primary)",color:"var(--color-text-primary)",fontWeight:view===v?500:400}}>
                  {v==="charts"?"Charts":"Table"}
                </button>
              ))}
            </div>
          )}
          <label htmlFor="reup" style={{display:"inline-flex",alignItems:"center",gap:5,padding:"5px 10px",border:"0.5px solid var(--color-border-secondary)",borderRadius:"var(--border-radius-md)",fontSize:12,cursor:"pointer",background:"var(--color-background-primary)",color:"var(--color-text-primary)"}}>
            ↑ Upload CSV
      ...
      {mainTab==="editor" ? (
        ...
        <>
          {/* No data state */}
          {!raw.length && (
            <div style={{border:"1.5px dashed var(--color-border-secondary)",borderRadius:"var(--border-radius-lg)",padding:"3rem",textAlign:"center",color:"var(--color-text-secondary)"}}>
          ...
          {raw.length > 0 && (
            <>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:"1rem",flexWrap:"wrap"}}>
                {allTabs.map(m=>(
                  <button key={m} onClick={()=>setActiveMet(m)} style={{padding:"5px 14px",fontSize:13,borderRadius:"var(--border-radius-md)",border:"0.5px solid var(--color-border-secondary)",cursor:"pointer",background:activeMet===m?"var(--color-text-primary)":"var(--color-background-primary)",color:activeMet===m?"var(--color-background-primary)":"var(--color-text-primary)",fontWeight:activeMet===m?500:400}}>
                    {m}
                  </button>
                ))}
                {!isComp && (
                  <div style={{display:"flex",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",overflow:"hidden",marginLeft:8}}>
                    <button onClick={()=>setPct(true)}  style={{padding:"5px 12px",fontSize:12,border:"none",cursor:"pointer",background:pct?"var(--color-background-secondary)":"var(--color-background-primary)",color:"var(--color-text-primary)",fontWeight:pct?500:400}}>% of volume</button>
                    <button onClick={()=>setPct(false)} style={{padding:"5px 12px",fontSize:12,border:"none",cursor:"pointer",background:!pct?"var(--color-background-secondary)":"var(--color-background-primary)",color:"var(--color-text-primary)",fontWeight:!pct?500:400}}># count</button>
                  </div>
                )}
                {isComp && (
                  <div style={{display:"flex",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",overflow:"hidden",marginLeft:8}}>
                    <button onClick={()=>setModelWt(true)}  style={{padding:"5px 12px",fontSize:12,border:"none",cursor:"pointer",background:modelWt?"var(--color-background-secondary)":"var(--color-background-primary)",color:"var(--color-text-primary)",fontWeight:modelWt?500:400}}>Model weights</button>
                    <button onClick={()=>setModelWt(false)} style={{padding:"5px 12px",fontSize:12,border:"none",cursor:"pointer",background:!modelWt?"var(--color-background-secondary)":"var(--color-background-primary)",color:"var(--color-text-primary)",fontWeight:!modelWt?500:400}}>Actual weights</button>
                  </div>
                )}
              </div>
              ...
              {!isComp && (
                <div style={{display:"flex",gap:6,marginBottom:"0.75rem",flexWrap:"wrap",alignItems:"center"}}>
                  <span style={{fontSize:12,color:"var(--color-text-secondary)",marginRight:2}}>Categories:</span>
                  {allCats.map(c=>{ const on=activeCats.includes(c); return (
                    <button key={c} onClick={()=>setActiveCats(p=>on?p.filter(x=>x!==c):[...p,c])}
                      style={{padding:"4px 10px",fontSize:12,borderRadius:"var(--border-radius-md)",border:`0.5px solid ${on?"var(--color-border-secondary)":"var(--color-border-tertiary)"}`,cursor:"pointer",background:on?"var(--color-background-primary)":"var(--color-background-secondary)",color:on?"var(--color-text-primary)":"var(--color-text-tertiary)",fontWeight:on?500:400}}>
                      {c}
                    </button>
                  )})}
                </div>
              ...
              <div style={{display:"flex",gap:6,marginBottom:"1.25rem",flexWrap:"wrap",alignItems:"center"}}>
                <span style={{fontSize:12,color:"var(--color-text-secondary)",marginRight:2}}>Segments:</span>
                {SEGMENT_COLS.map((col,i)=>{ const on=activeSegs.includes(col); return (
                  <button key={col} onClick={()=>setActiveSegs(p=>on?p.filter(c=>c!==col):[...p,col])}
                    style={{display:"flex",alignItems:"center",gap:5,padding:"4px 10px",fontSize:12,borderRadius:"var(--border-radius-md)",border:`0.5px solid ${on?SEG_COLORS[i]:"var(--color-border-tertiary)"}`,cursor:"pointer",background:on?"var(--color-background-primary)":"var(--color-background-secondary)",color:on?SEG_COLORS[i]:"var(--color-text-tertiary)"}}>
                    <span style={{width:8,height:8,borderRadius:2,background:on?SEG_COLORS[i]:"var(--color-border-secondary)"}}></span>{dn(col)}
                  </button>
                )})}
              </div>
                ...
                activeMet ? (
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:12}}>
                    {allCats.filter(c=>activeCats.includes(c)).map(cr=>(
                      <div key={cr} style={{background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"1rem"}}>
                        <div style={{fontSize:13,fontWeight:500,marginBottom:2}}>{cr}</div>
                        <div style={{fontSize:11,color:"var(--color-text-tertiary)",marginBottom:10}}>{activeMet}{!isVol(activeMet)?" stacked on volume":""}</div>
                      ...
                      <thead>
                        <tr>
                          {["Cancellation Reason","Metric",...ordSegs].map(h=>(
                            <th key={h} style={{background:"var(--color-background-secondary)",fontWeight:500,padding:"10px 16px",textAlign:["Cancellation Reason","Metric"].includes(h)?"left":"right",whiteSpace:"nowrap",borderBottom:"0.5px solid var(--color-border-tertiary)",color:SEGMENT_COLS.includes(h)?SEG_COLORS[SEGMENT_COLS.indexOf(h)]:"var(--color-text-secondary)",fontSize:11,textTransform:"uppercase",letterSpacing:"0.04em"}}>
                          ...
                          return filtered.map((row,i)=>{
                            ...
                            return (
                              <tr key={i} style={{background:i%2===0?"var(--color-background-primary)":"var(--color-background-secondary)"}}>
                                <td style={{padding:"9px 16px",borderBottom:isLast?"1px solid var(--color-border-secondary)":"0.5px solid var(--color-border-tertiary)",fontWeight:500,whiteSpace:"nowrap"}}>{cr||"—"}</td>
                                <td style={{padding:"9px 16px",borderBottom:isLast?"1px solid var(--color-border-secondary)":"0.5px solid var(--color-border-tertiary)",whiteSpace:"nowrap",color:"var(--color-text-secondary)"}}>{rm||"—"}</td>
                                {ordSegs.map(seg=>{
                                  const raw2=pv(row[seg])
                                  const vol=getVol(cr,seg)
                                  let disp
                                  ...
                                  return <td key={seg} style={{padding:"9px 16px",borderBottom:isLast?"1px solid var(--color-border-secondary)":"0.5px solid var(--color-border-tertiary)",textAlign:"right",whiteSpace:"nowrap",fontVariantNumeric:"tabular-nums",color:bad?"#C0392B":"var(--color-text-primary)",fontWeight:bad?500:400}}>{disp}</td>
                                })}
                              </tr>
                            )
                          })
